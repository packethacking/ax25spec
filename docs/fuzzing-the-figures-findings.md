...
Candidate fix, for the working group rather than this report: either an exit to Connected from the figc4.5 `I_received` arm when the acknowledgement clears the window (direwolf's shape), or a T3 arm in figc4.5, and the RC reset of #9 alongside. Whether packet.net's own runtime is exposed on a live link (a session that goes quiet after a partial-acknowledgement recovery losing its T3 keepalive) is being checked with a deterministic harness test at the time of writing.
...
2. **File H1** once reproduced, with the direwolf note as the second piece of evidence, and propose the fix shape above together with #9.
3. **Close #38.**
4. **Extend the explorer's move set** with `DL_FLOW_OFF_request` / `DL_FLOW_ON_request` and a v2.0 peer model that answers SABME with FRMR or DM, so #43, #45 and #48 become reachable as invariant violations rather than trace expectations.
5. **Add a golden trace for #48** (DM to a SABME) and for the H1 scenario, so a future figure change is noticed.
6. **Spend the next citation pass on Timer Recovery** and the management data-link pages.
7. **A numeric-timer extension** would bring #41 and the T1 backoff behaviour into reach; it is the one class no checker here can express.
